...
fn strings_hexadecimal_escape_sequence() {
    assert_lexer_eq!(
        ...
        vec![string_literal(r"hello JavaScript".to_string(), 0, 24)]
    );
}
...
fn strings_unicode_escape_sequence() {
    assert_lexer_eq!(
        r"'hello\u0020world'",
        vec![string_literal(r"hello world".to_string(), 0, 18)]
    );
}
...
fn strings_code_points_escape_sequence() {
    assert_lexer_eq!(
        ...
        vec![string_literal("hello world 😇🔆".to_string(), 0, 32)]
    );
}
...
fn strings_octal_legacy_escape_sequence() {
    assert_lexer_eq!(
        r"'hello \127\117rld'",
        vec![string_literal("hello WOrld".to_string(), 0, 19)]
    );
}
...
#[test]
fn strings_empty() {
    assert_lexer_eq!(r#""""#, vec![string_literal("".to_string(), 0, 2)]);
    assert_lexer_eq!(r"''", vec![string_literal("".to_string(), 0, 2)]);
}

#[test]
fn strings_non_english_chars() {
    assert_lexer_eq!("'دیوانه'", vec![string_literal("دیوانه".to_string(), 0, 8)]);
    assert_lexer_eq!("'a℮'", vec![string_literal("a℮".to_string(), 0, 4)]);
    assert_lexer_eq!("'℘'", vec![string_literal("℘".to_string(), 0, 3)]);
    assert_lexer_eq!("'a᧚'", vec![string_literal("a᧚".to_string(), 0, 4)]);
    assert_lexer_eq!(
        "'б И Й К Л О Ф Ц Ш Э ж з'",
        vec![string_literal("б И Й К Л О Ф Ц Ш Э ж з".to_string(), 0, 25)]
    );
}
