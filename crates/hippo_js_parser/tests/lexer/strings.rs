...
fn strings_simple() {
    assert_lexer_eq!(
        r"'hello world'",
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 13)]
    );

    assert_lexer_eq!(
        r"'hello\n\tworld'",
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 16)]
    );
}
...
fn strings_hexadecimal_escape_sequence() {
    assert_lexer_eq!(
        ...
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 24)]
    );
}
...
fn strings_unicode_escape_sequence() {
    assert_lexer_eq!(
        r"'hello\u0020world'",
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 18)]
    );
}
...
fn strings_with_code_points() {
    assert_lexer_eq!(
        ...
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 23)]
    );
}
...
fn strings_with_complex_graphemes() {
    assert_lexer_eq!(
        ...
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 46)]
    );
}
...
fn strings_non_english() {
    assert_lexer_eq!(
        "'دیوانه'",
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 8)]
    );
    assert_lexer_eq!(
        "'a℮'",
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 4)]
    );
    assert_lexer_eq!(
        "'℘'",
    ...
    assert_lexer_eq!(
        ...
        vec![Token::new(TokenKind::StringLiteral("".to_string()), 0, 25)]
    );
}
